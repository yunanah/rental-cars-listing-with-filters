...
function CarList() {
  ...
  const carRegions = ['광주', '대전', '대구/경북', '부산/창원', '서울/경기/인천']

  // eslint-disable-next-line react-hooks/exhaustive-deps
  useEffect( async () => {

    await axios.get('http://localhost:8080/carClasses')
    .then((result) => {
      
      let timer = setTimeout(()=>{ setLoading(false) }, 2000) // 데이터 로딩 종료
      // 더보기 버튼, 재고 상태 초기화
      setMore(true) 
      setStock(true)
      ...
      if (cloneData.length < (morePress+1)*5) { // 차량 데이터 크기에 따라 0-재고없음 , 더보기버튼 보이는지 여부 처리
        setMore(false)
      }
      ...
      } else if (filter == 2) { // 따로 수치가 없고 인기 태그 여부로 배치했습니다
        let tempArr = []
    ...
    <div className={`body ${(detail ? 'dimmed' : '')}`}>
        <div className="list-view">
          <div className="header">
            <Link to="/"><FontAwesomeIcon className="go-back" icon={faArrowLeft}/></Link>
            <h2>추천 차량</h2>
            <div className="buttons">
              <div className="select-group is-active">
                <select onChange={handleSelect} value={filter} className="form-select">
              ...
              <div className="show-version-btn">
                <FontAwesomeIcon onClick={()=>{setViewMany(!viewMany)}} className="icon-btn" icon={viewMany ? faAlignJustify : faSquare} />  
              </div>
            </div>
            <div className="types">
                  ...
                  <div className="items">
                    {
                      carSizes.map((a, i) => {
                        return (
                          <span className={`item ${(sizes[i] ? 'selected' : '')}`} onClick={()=> {
                  ...
                  <div className="items">
                    {
                      carRegions.map((a, i) => {
                        return (
                          <span className={`item ${(regions[i] ? 'selected' : '')}`} onClick={()=>{
          ...
          { 
            ...
            : !stock ? (<div className='no-stock'>차량 없음</div>)
            : (
              <>
                {
                  // render 차량 Card 컴포넌트
                  cars.map((a, i) => {
                    return <Card className="card" viewMany={viewMany} setScroll={setScroll} setCarInfo={setCarInfo} setDetail={setDetail} car={cars[i]} key={i} />
                  })
                }
                ...
                
                {
                  // 상세정보 모달창 
                  detail
                  ? (<Detail setDetail={setDetail} carInfo={carInfo}/>)
                  : null
                }
              </>
            )
...
function Card(props) {
  ...
  // render
  return (
    <div className={`box ${viewMany ? 'short' : ''}`} onClick={() => {
      setScroll(window.scrollY)
      axios.get(`http://localhost:8080/carClasses/${car.carClassId}`)
      .then((result) => {
      ...
      <div className="info">
        ...
        <p>
          {car.price.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",")}원 <span className="discount">{car.discountPercent !== 0 ? `(-${car.discountPercent}%)` : null}</span> <br />
          <span className="sub-info">{car.year}년 | {car.drivingDistance >= 10000 ? car.drivingDistance/10000 + '만km' : car.drivingDistance/1000 + '천km'}
          &nbsp;| {car.regionGroups.join(', ')}</span>
        </p>
      </div>
    </div>
...
function Detail(props) {
  ...
  //render
  return (
    <div className="modalBackground" onClick={() => {setDetail(false)}}>
      <div className="modalBox">
        <span className="close" onClick={() => {setDetail(false)}}><FontAwesomeIcon icon={faTimes}/></span>
